...
describe("TrustDistribute", async function () {
  it("supports precision of no zeros", async function () {
    this.timeout(0);
  ...
  it("supports precision of four zeros", async function () {
    this.timeout(0);
    ...
    const reserveInit = ethers.BigNumber.from("2000" + Util.fourZeros);
    const redeemInit = ethers.BigNumber.from("2000" + Util.fourZeros);
    const initialValuation = ethers.BigNumber.from("10000" + Util.fourZeros);
    const totalTokenSupply = ethers.BigNumber.from("2000" + Util.fourZeros);

    const minimumCreatorRaise = ethers.BigNumber.from("100" + Util.fourZeros);
    const seederFee = ethers.BigNumber.from("100" + Util.fourZeros);
    const seederUnits = 0;
    const seederCooldownDuration = 0;
