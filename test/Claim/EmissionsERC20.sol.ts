...
describe("EmissionsERC20", async function () {
  it.only("should calculate correct emissions amount (if division is performed on final result)", async function () {
    ...
    // FN uses constant 0
    const valTierAddress = op(Opcode.val, 1);
    const valBaseRewardPerTier = op(Opcode.val, 2);
    const valBlocksPerYear = op(Opcode.val, 3);
    const valBOne = op(Opcode.val, 4);
    const valBOneReward = op(Opcode.val, 5);
