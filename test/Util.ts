...
export const balancerDeploy = async ():Promise<[RightsManager, CRPFactory, BFactory]> => {
...
export const poolContracts = async (signers:any,pool:RedeemableERC20Pool):Promise<[ConfigurableRightsPool,BPool]> => {
  const crp = new ethers.Contract(await pool.crp(), crpJson.abi, signers[0]) as ConfigurableRightsPool
  const bPool = new ethers.Contract(await crp.bPool(), bPoolJson.abi, signers[0]) as BPool
  return [crp, bPool]
}