...
describe("ERC20TransferTier", async function () {
  ...
  it("should respect block number on same tier", async () => {
    ...
    assert(block2 === block1 + 1, `block2 not 1 more than block1: ${block1} ${block2}`)
    assert(report2.eq(report1), `report2 not equal to report1: ${report1} ${report2}`)
    ...
    assert(block3 === block2 + 1, `block3 not 1 more than block2: ${block2} ${block3}`)
    assert(report3.eq(report2), `report3 not equal to report2: ${report2} ${report3}`)
  })
