...
import { basicDeploy, eighteenZeros, sixteenZeros, sixZeros, tenZeros } from "../../../utils";
...
describe("FixedPointMathTest scaling a number", async function () {
  ...
  it("should scale a fixed point decimal UP to scale 18 for a_ != scaled_ * b_", async () => {
    ...
    const aDecimals_ = 20; 
    const result = await fixedPointMathTest.scale18(a_, aDecimals_,ROUND_UP); 
    const expectedResult = ethers.BigNumber.from(1 + sixteenZeros + "33").add(ethers.BigNumber.from(1)); 
    assert(result.eq(expectedResult));
  });
  ...
  it("should scale a fixed point decimal UP to scale N", async () => {
    ...
    const result = await fixedPointMathTest.scaleN(a_, targetDecimals_,ROUND_UP); 
    const expectedResult = ethers.BigNumber.from(10 + tenZeros).add(ethers.BigNumber.from(1));
    assert(result.eq(expectedResult));
  });
  ...
  it("should scale a fixed point decimal UP by scale N (scaleBy_ < 0) ", async () => {
    ...
    const result = await fixedPointMathTest.scaleBy(a_, scaleBy_,ROUND_UP);
    const expectedResult = ethers.BigNumber.from(1 + sixteenZeros).add(ethers.BigNumber.from(1));
    assert(result.eq(expectedResult));
  });
  ...
  it("should scale ratio UP for aDecimals > bDecimals", async () => {
    ...
    const result = await fixedPointMathTest.scaleRatio(ratio , aDecimals , bDecimals ,ROUND_UP);
    const expectedResult = ethers.BigNumber.from(1 + tenZeros + "003675").add(ethers.BigNumber.from(1));
    assert(result.eq(expectedResult));
  }); 
  ...
  it("should scale ratio UP for aDecimals < bDecimals", async () => {
    ...
    const result = await fixedPointMathTest.scaleRatio(ratio , aDecimals , bDecimals ,ROUND_UP);
    const expectedResult = ethers.BigNumber.from(1 + tenZeros + "00367589" + "00");
    assert(result.eq(expectedResult));
  });
  ...
  it("should scale ratio DOWN for aDecimals > bDecimals", async () => {
    ...
    const result = await fixedPointMathTest.scaleRatio(ratio , aDecimals , bDecimals ,ROUND_DOWN);
  ...
  it("should scale ratio by 18 OOM for bDecimals = 0", async () => {
    ...
    const result = await fixedPointMathTest.scaleRatio(ratio , aDecimals , bDecimals ,ROUND_TO_ZERO); 
    const expectedResult = ethers.BigNumber.from(1);
    assert(result.eq(expectedResult));
  });
  ...
  it("should scale ratio by 18 OOM for aDecimals = 0", async () => {
    ...
    const result = await fixedPointMathTest.scaleRatio(ratio , aDecimals , bDecimals ,ROUND_TO_ZERO);  
    const expectedResult = ethers.BigNumber.from(1 + tenZeros + "00367589" + eighteenZeros);
    assert(result.eq(expectedResult));
  }); 
  ...
  it("should scale down by specified decimals and round UP", async () => {
    ...
    const result = await fixedPointMathTest.scaleDown(a_ , scaleDownBy ,ROUND_UP);
    const expectedResult = ethers.BigNumber.from(1 + sixteenZeros);
    assert(result.eq(expectedResult)); 
  ...
  it("should scale down by specified decimals and round DOWN", async () => {
    ...
    const result = await fixedPointMathTest.scaleDown(a_ , scaleDownBy ,ROUND_DOWN);
    const expectedResult = ethers.BigNumber.from(1 + sixteenZeros);
    assert(result.eq(expectedResult)); 
  ...
  it("should scale down by 18 OOMs", async () => {
    ...
    const result = await fixedPointMathTest.scaleDown(a_ , scaleDownBy ,ROUND_TO_ZERO); 
    const expectedResult = ethers.BigNumber.from(1);
    assert(result.eq(expectedResult)); 
