...
describe("FlowERC20 previewFlow tests", async function () {
  ...
  it("should preview defined flow IO for ERC1155 (multi element arrays)", async () => {
    ...
    const CAN_TRANSFER = () =>
      op(Opcode.read_memory, memoryOperand(MemoryType.Constant, 1));
    const SENTINEL_ERC20 = () =>
      op(Opcode.read_memory, memoryOperand(MemoryType.Constant, 16));
    ...
    const MINT_AMOUNT = () =>
      op(Opcode.read_memory, memoryOperand(MemoryType.Constant, 14));
    const BURN_AMOUNT = () =>
      op(Opcode.read_memory, memoryOperand(MemoryType.Constant, 15));
