...
describe("FlowERC20 flow tests", async function () {
  ...
  it("should mint and burn tokens per flow in exchange for another token (e.g. ERC20 )", async () => {
    ...
    const flowERC20IOMint: FlowERC20IOV1Struct = {
      mints: [
        {
          account: you.address,
          amount: mintMint,
    ...
    const flowERC20IOBurn: FlowERC20IOV1Struct = {
      mints: [
        {
          account: you.address,
          amount: mintBurn,
    ...
    const { sources: sourceFlowIOMint, constants: constantsFlowIOMint } =
      await standardEvaluableConfig(
        ...
        you: context<0 0>(),
        me: context<0 1>(),
        flowio-input-token: ${flowERC20IOMint.flow.erc20[0].token},
        flowio-output-token: ${flowERC20IOMint.flow.erc20[1].token},
        flowio-input-erc20-amount: ${flowERC20IOMint.flow.erc20[0].amount},
        flowio-output-erc20-amount: ${flowERC20IOMint.flow.erc20[1].amount},
        mintamount: ${mintMint},
        burnamount: ${burnMint},
    ...
    const { sources: sourceFlowIOBurn, constants: constantsFlowIOBurn } =
      await standardEvaluableConfig(
        ...
        you: context<0 0>(),
        me: context<0 1>(),
        flowio-input-token: ${flowERC20IOBurn.flow.erc20[0].token},
        flowio-output-token: ${flowERC20IOBurn.flow.erc20[1].token},
        flowio-input-erc20-amount: ${flowERC20IOBurn.flow.erc20[0].amount},
        flowio-output-erc20-amount: ${flowERC20IOBurn.flow.erc20[1].amount},
        mintamount: ${mintBurn},
        burnamount: ${burnBurn},
    ...
    assert(
      meMintBalanceIn.eq(await flowERC20IOMint.flow.erc20[0].amount),
      `wrong balance for me (flow contract)
      expected  ${flowERC20IOMint.flow.erc20[0].amount}
      got       ${meMintBalanceIn}`
    );
    ...
    assert(
      meBurnBalanceIn.eq(await flowERC20IOMint.flow.erc20[0].amount),
      `wrong balance for me (flow contract)
      expected  ${flowERC20IOMint.flow.erc20[0].amount}
      got       ${meBurnBalanceIn}`
    );
    ...
    assert(
      youBurnBalanceOut.eq(await flowERC20IOBurn.flow.erc20[1].amount),
      `wrong balance for you (signer1 contract)
      expected  ${flowERC20IOBurn.flow.erc20[1].amount}
  ...
  it("should not be able to access values set in a flow across different flows", async () => {
    ...
    const { sources: sourceFlowIOA, constants: constantsFlowIOA } =
      ...
      me: context<0 1>(),
      key: ${key},
      flowtransfer-you-to-me-erc20-token: ${flowERC20IO.flow.erc20[0].token},
    ...
    assert(
      meABalanceIn.eq(await flowERC20IO.flow.erc20[0].amount),
      `wrong balance for me (flow contract)
      expected  ${flowERC20IO.flow.erc20[0].amount}
      got       ${meABalanceIn}`
    );
    ...
    assert(
      youABalanceOut.eq(await flowERC20IO.flow.erc20[1].amount),
      `wrong balance for you (signer1 contract)
      expected  ${flowERC20IO.flow.erc20[1].amount}
      got       ${youABalanceOut}`
    );
