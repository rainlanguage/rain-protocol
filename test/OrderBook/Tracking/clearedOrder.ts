...
describe("OrderBook tracking order funds cleared", async function () {
  ...
  it("should record cleared order funds", async function () {
    ...
    const askSource = concat([
      op(Opcode.CONTEXT, 0x0003), // tracking triggered by reading from context
  ...
  it("should record cleared order funds and allow reading via context (e.g. asker throttles output of their tokens to 5 tokens per block)", async function () {
    const signers = await ethers.getSigners();
  ...
  it("should record cleared order funds and allow reading via opcode (e.g. asker throttles output of their tokens to 5 tokens per block)", async function () {
    const signers = await ethers.getSigners();
