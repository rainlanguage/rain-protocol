...
describe.only("OrderBook tracking counterparty funds cleared", async function () {
  ...
  it("should record cleared counterparty funds and allow reading from getter", async function () {
    ...
    const askSource = concat([
      op(Opcode.CONTEXT, 0x0004), // tracking triggered by reading from context
    ...
    const expectedClearedCounterpartyValue_ = (
      depositConfigStructBob.amount as BigNumber
    )
      .mul(bidPrice)
      .div(ONE);
  ...
  it("should record cleared counterparty funds and allow reading via context (e.g. asker throttles output of their tokens to 5 tokens per block per bidder)", async function () {
    const signers = await ethers.getSigners();
  ...
  it("should record cleared counterparty funds and allow reading via opcode (e.g. asker throttles output of their tokens to 5 tokens per block per bidder)", async function () {
    const signers = await ethers.getSigners();
