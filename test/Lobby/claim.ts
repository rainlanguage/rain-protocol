...
describe("Lobby Tests claim", async function () {
  ...
  it("should ensure player are able to claim on a good path (shares add up to 1e18)", async function () { 
   ...
   // Bob Joins lobby 
    ...
    // Alice Claims Amount
    const aliceClaimTx = await Lobby.connect(alice).claim([aliceShares], signedContexts2);    
    ...
    // Bob Claims Amount
    const bobClaimTx = await Lobby.connect(bob).claim([bobShares], signedContexts2);    
