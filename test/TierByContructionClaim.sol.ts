...
describe("TierByConstructionClaim", async function () {
  ...
  it('should set tierContract and minimumTier on construction', async () => {
    const tierByConstructionClaim = await tierByConstructionClaimFactory.deploy(readWriteTier.address, Tier.FOUR) as TierByConstructionClaim
  ...
  describe("Claim", async () => {
    describe('should require minimum tier as per TierByConstruction logic (tier held continuously since contract construction) - failure case', async () => {
      it('min tier after construction', async () => {
        ...
        await readWriteTier.connect(alice).setTier(alice.address, Tier.FOUR, []) // after construction
      ...
      it('greater than min tier before construction', async () => {
        await readWriteTier.connect(alice).setTier(alice.address, Tier.FIVE, []) // before construction
      ...
      it('less than min tier before construction', async () => {
        await readWriteTier.connect(alice).setTier(alice.address, Tier.THREE, []) // before construction
