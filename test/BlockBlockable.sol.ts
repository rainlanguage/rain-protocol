...
describe("BlockBlockable", async function() {
    it("should be (un)blockable", async function() {
        ...
        assert(ethers.BigNumber.from(0).eq(await blockable.getUnblockBlock()), 'wrong starting unblock block 1')
        ...
        assert(ethers.BigNumber.from(0).eq(await moreBlockable.getUnblockBlock()), 'wrong starting unblock block 2')
