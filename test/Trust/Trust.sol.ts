...
describe("Trust", async function () {
  ...
  it("should work happily if griefer sends small amount of reserve to contracts and signers including SeedERC20 contract", async function () {
    ...
    // IF SEEDERS DO NOT PULL, SEEDER REDEMPTION SHOULD NOT WORK.
    // const allowance = await reserve.allowance(trust.address, seeder);
    // // seeder1 pulls erc20
    // await seederContract
    //   .connect(seeder1)
    ...
    const seeder1Balance = await reserve.balanceOf(seeder1.address);
    const seeder2Balance = await reserve.balanceOf(seeder2.address);

    assert(
      seeder1Balance.eq("877599999"),
      `RESERVE_BALANCE guard was not triggered after contract griefed
      expected  877599999
      got       ${seeder1Balance}`
    );
    assert(
      seeder2Balance.eq("1301399999"),
      `RESERVE_BALANCE guard was not triggered after contract griefed
      expected  1301399999
      got       ${seeder2Balance}`
    );

    // IF SIGNERS DO NOT PULL, TOKEN REDEMPTION SHOULD NOT WORK.
    // // signer1 pulls erc20 into RedeemableERC20 contract
    // await token
    //   .connect(signer1)
    //   .pullERC20(
    ...
    await Util.assertError(
      async () =>
        await token
          ...
          .redeem([reserve.address], await token.balanceOf(signer1.address)),
      "ZERO_TRANSFER",
      "ZERO_TRANSFER guard was not triggered after contract griefed"
    );
  });
