...
export async function verifyContract(
  ...
  if (hre.network.name !== "hardhat") {
    ...
    await _sendVerification(apiUrl, apiKey, body);
  }
}

async function _sendVerification(url: string, apiKey: string, body: any) {
...
async function getContractInfo(contractName_: string) {
  // Get the generated Input JSON file (solt)
  const soltFile = fetchSoltFile(contractName_);
...
const fetchSoltFile = (contractName_: string) => {
  const _path = path.resolve(
    __dirname,
    `../../solt/solc-input-${contractName_.toLowerCase()}.json`
  );
