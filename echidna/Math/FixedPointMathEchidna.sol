...
contract FixedPointMathEchidna {
    ...
    function ScaleRatio(
        ...
        // Make sure that
        try
            _fixedPointMathTest.scaleRatio(
                ratio_,
                aDecimals_.toUint256(),
                bDecimals_.toUint256(),
                Math.Rounding(rounding_ % 3)
            )
        returns (uint256 valueScaled) {
            ...
            if (scaleBy_ == 0) {
                ...
                if (scaleBy_ > 0) {
                    ...
                    if (
                        Math.Rounding(rounding_ % 3) == Math.Rounding.Up &&
                        ratio_ != valueExpected * b_
                    ) {
                        valueExpected += 1;
                    }
                }
    ...
    function FixedPointMul(
        ...
        uint256 valueCalculated = _fixedPointMathTest.fixedPointMul(
            a_,
            b_,
            Math.Rounding(rounding_ % 3)
        );
        ...
        uint256 valueExpected = Math.mulDiv(
            a_,
            b_,
            FP_ONE,
            Math.Rounding(rounding_ % 3)
    ...
    function FixedPointDiv(
        ...
        uint256 valueCalculated = _fixedPointMathTest.fixedPointDiv(
            a_,
            b_,
            Math.Rounding(rounding_ % 3)
        );
