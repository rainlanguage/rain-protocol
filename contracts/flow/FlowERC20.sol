...
contract FlowERC20 is ReentrancyGuard, StandardVM, ERC20 {
    ...
    mapping(uint => uint256) private flowTimes;
    ...
    function initialize(FlowERC20Config calldata config_)
        external
        initializer
    {
    ...
    function _rebaseRatio(VMState memory state_) internal view returns (uint) {
    ...
    function rebaseInput(VMState memory state_, uint input_) internal view returns (uint) {
    ...
    function rebaseOutput(uint output_) internal view returns (uint) {
    ...
    function balanceOf(address account_) public view virtual override returns (uint) {
        return rebaseOutput(super.balanceOf(account_));
    }

    function transferPreflight(address from_, address to_, uint amount_) internal view returns (uint amountRebased_) {
        VMState memory state_ = _loadVMState();
        amountRebased_ = rebaseInput(state_, amount_);
        state_.context = LibUint256Array.arrayFrom(uint(uint160(from_)), uint(uint160(to_)), amount_, amountRebased_);
        require(state_.eval(CAN_TRANSFER_ENDPOINT).peek() > 0, "INVALID_TRANSFER");
    }

    function transfer(address to_, uint amount_) public virtual override returns (bool) {
        return super.transfer(to_, transferPreflight(msg.sender, to_, amount_));
    }

    function transferFrom(address from_, address to_, uint amount_) public virtual override returns (bool) {
        return super.transferFrom(from_, to_, rebaseInput(_loadVMState(), amount_));
    }

    function allowance(address owner_, address spender_) public view virtual override returns (uint) {
        return rebaseOutput(super.allowance(owner_, spender_));
    }

    function approve(address spender_, uint amount_) public virtual override returns (bool) {
        return super.approve(spender_, rebaseInput(_loadVMState(), amount_));
    }

    function increaseAllowance(address spender_, uint addedValue_) public virtual override returns (bool) {
        return super.increaseAllowance(spender_, rebaseInput(_loadVMState(), addedValue_));
    }

    function decreaseAllowance(address spender_, uint subtractedValue_) public virtual override returns (bool) {
        return super.decreaseAllowance(spender_, rebaseInput(_loadVMState(), subtractedValue_));
    }

    function previewFlow(SourceIndex flow_, uint id_) public view returns (FlowERC20IO memory flowERC20IO_) {
        require(SourceIndex.unwrap(flow_) > SourceIndex.unwrap(CAN_FLOW_ENDPOINT), "FLOW_OOB");
        VMState memory state_ = _loadVMState(LibUint256Array.arrayFrom(SourceIndex.unwrap(flow_), id_));
        require(SourceIndex.unwrap(flow_) < state_.compiledSources.length, "FLOW_OOB");
    ...
    function flow(SourceIndex flow_, uint id_) external returns (FlowERC20IO memory) {
