...
library OpIOrderBookV1ClearedOrder {
    ...
    function f(uint256 orderbook_, uint orderHash_) internal view returns (uint256) {
        return uint256(uint160(IOrderBookV1(address(uint160(orderbook_))).clearedOrder(orderHash_)));
    }
