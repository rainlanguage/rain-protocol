...
library LibInterpreter {
    ...
    function logArray(uint[] memory array_) internal view {
        unchecked {
    ...
    function debug(
        ...
        if (debugStyle_ == DebugStyle.Source) {
            ...
            if (debugStyle_ == DebugStyle.Stack) {
                uint length_ = state_.stackBottom.toIndex(stackTop_);
                uint[] memory array_ = StackTop
                    .unwrap(stackTop_.down(length_))
                    .copyToNewUint256Array(length_);
                logArray(array_);
            } else if (debugStyle_ == DebugStyle.Constant) {
                uint[] memory array_ = state_.constantsBottom.down().asUint256Array();
                logArray(array_);
            } else {
                unchecked {
                    for (uint i_ = 0; i_ < state_.context.length; i_++) {
                        logArray(state_.context[i_]);
                    }
                }
            }
        }
    ...
    function buildStateBytes(
        ...
        unchecked {
            (uint256 scratch_, uint stackLength_) = interpreterIntegrity_
                .ensureIntegrity(
                    config_.sources,
