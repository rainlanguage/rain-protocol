...
/// some other tokens held by the contract.
/// For example, if the `RedeemableERC20` token contract holds 100 000 USDC
/// then a holder of the redeemable token
/// can burn some of their tokens to receive a % of that USDC. If they redeemed
/// (burned) an amount equal to 10% of the redeemable token supply then
/// they would receive 10 000 USDC.
///
/// Up to 8 redeemable tokens can be registered on the redeemable contract.
/// These will be looped over by default in
/// the `redeem` function. If there is an error during redemption or more than
/// 8 tokens are to be redeemed, there is a `redeemSpecific` function that
/// allows the caller to specify exactly which of the redeemable tokens they
/// want to receive.
...
/// contract, NOT an externally owned account.
/// The `Trust` contract will add the minimum possible senders/receivers to
/// facilitate the AMM trading and redemption.
