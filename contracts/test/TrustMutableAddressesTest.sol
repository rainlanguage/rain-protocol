...
/// @title TrustMutableAddressesTest
/// Test contract that replicates `Trust` contract except for extra
/// `update` functions which allow the reserve, token and crp addresses to be
/// changed.
/// This 'malicious' Trust contract is for test that `TrustEscrow` can prevent
/// such a contract from changing these addresses.
...
contract TrustMutableAddressesTest is Phased, ISale {
    ...
    // a 'malicious' function which allows the reserve address to be
    // modified after initialisation
    function updateReserve(address reserve_) external {
      _reserve = IERC20(reserve_);
    }

    // a 'malicious' function which allows the token address to be modified
    // after initialisation
    function updateToken(address token_) external {
      _token = RedeemableERC20(token_);
    }

    // a 'malicious' function which allows the crp address to be modified
    // after initialisation
