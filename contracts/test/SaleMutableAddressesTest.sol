...
/// Test contract that replicates `Sale` contract except for extra
/// `updateReserve()` and `updateToken()` functions which allow the
/// reserve and token addresses to be changed.
/// This 'malicious' Sale contract is for test that `SaleEscrow` can prevent
/// such a contract from changing these addresses.
// solhint-disable-next-line max-states-count
contract SaleMutableAddressesTest is
    Initializable,
    Cooldown,
    RainVM,
    VMState,
