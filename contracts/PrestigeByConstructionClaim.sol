 ...
 */
contract PrestigeByConstructionClaim is PrestigeByConstruction {
     ...
     */
    function claim(address account, bytes memory data)
        external
        onlyStatus(account, IPrestige.Status.GOLD)
    {
        require(!claims[account], "ERR_MULTI_MINT");
        claims[account] = true;
        _processClaim(account, prestige.statusReport(account), data);
    }

    function _processClaim(
        address account,
        uint256 statusReport,
        bytes memory data
    )
        internal virtual
    {}
