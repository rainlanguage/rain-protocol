...
import {SenderOps, SENDER_OPS_LENGTH} from "./SenderOps.sol";
import {ThisOps, THIS_OPS_LENGTH} from "./ThisOps.sol";
...
uint256 constant SENDER_OPS_START = MATH_OPS_START + MATH_OPS_LENGTH;
uint256 constant THIS_OPS_START = SENDER_OPS_START + SENDER_OPS_LENGTH;
uint256 constant TIER_OPS_START = THIS_OPS_START + THIS_OPS_LENGTH;
...
library AllStandardOps {
    function applyOp(
        ...
        unchecked {
            ...
            } else if (opcode_ < SENDER_OPS_START) {
                MathOps.applyOp(state_, opcode_ - MATH_OPS_START, operand_);
            } else if (opcode_ < THIS_OPS_START) {
                SenderOps.applyOp(state_, opcode_ - SENDER_OPS_START, operand_);
            } else if (opcode_ < TIER_OPS_START) {
                ThisOps.applyOp(state_, opcode_ - THIS_OPS_START, operand_);
            } else {
