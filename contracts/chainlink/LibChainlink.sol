...
library LibChainlink {
    ...
    function price(address feed_, uint staleAfter_) external view returns (uint256 price_) {
        (, int256 answer_, , uint256 updatedAt_, ) = AggregatorV3Interface(feed_).latestRoundData();
        ...
        price_ = answer_.toUint256().scale18(AggregatorV3Interface(feed_).decimals());
    }
}