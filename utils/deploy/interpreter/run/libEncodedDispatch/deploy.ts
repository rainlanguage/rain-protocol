...
export const libEncodedDispatchDeploy = async () => {
  const libEncodedDispatchFactory = await ethers.getContractFactory("LibEncodedDispatchTest");
  const libEncodedDispatch = (await libEncodedDispatchFactory.deploy()) as LibEncodedDispatchTest;
